...
const SSHCommandFormModal: React.FC<SSHCommandFormModalProps> = ({
  ...
  return (
    <Modal
      title={
        isEditing
          ? t('jadeTree.command.editTitle')
          : t('jadeTree.command.createTitle')
      }
      open={open}
      onCancel={onCancel}
      onOk={onSubmit}
    >
      <Form form={form} layout='vertical'>
        ...
        <Form.Item
          name='content'
          label={t('jadeTree.command.content')}
          rules={[{ required: true, message: t('jadeTree.form.required') }]}
        >
          <Input.TextArea
            rows={5}
            placeholder={t('jadeTree.command.contentPlaceholder')}
          />
        </Form.Item>
