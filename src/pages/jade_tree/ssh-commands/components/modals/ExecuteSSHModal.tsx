...
const ExecuteSSHModal: React.FC<ExecuteSSHModalProps> = ({
  ...
  return (
    <Modal title={t('jadeTree.command.executeTitle', { name: target?.name || '-' })} open={open} width={720} onCancel={onCancel} onOk={onSubmit}>
      <Form form={form} layout='vertical'>
        <Form.Item name='host' label={t('jadeTree.execute.host')} rules={[{ required: true, message: t('jadeTree.form.required') }]}>
          <Input placeholder={t('jadeTree.execute.hostPlaceholder')} />
        </Form.Item>
        <Form.Item name='port' label={t('jadeTree.execute.port')} initialValue={22}>
          <InputNumber className='w-full' min={1} placeholder={t('jadeTree.execute.portPlaceholder')} />
        </Form.Item>
        <Form.Item name='username' label={t('jadeTree.execute.username')} rules={[{ required: true, message: t('jadeTree.form.required') }]}>
        ...
        <Form.Item name='privateKey' label={t('jadeTree.execute.privateKey')}>
          <Input.TextArea rows={4} placeholder={t('jadeTree.execute.privateKeyPlaceholder')} />
        </Form.Item>
        <Form.Item name='timeoutSeconds' label={t('jadeTree.execute.timeout')}>
          <InputNumber className='w-full' min={1} placeholder={t('jadeTree.execute.timeoutPlaceholder')} />
        </Form.Item>
      </Form>
      {result ? (
        <Card size='small' title={`${t('jadeTree.execute.result')} (exitCode=${result.exitCode ?? '-'})`}>
