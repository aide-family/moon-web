// 通用翻译（所有页面共享）
// 中英文翻译写在一个文件中，便于管理和维护

export const zhCN = {
  // 主题相关
  'theme.light': '亮色主题',
  'theme.dark': '暗色主题',
  'theme.system': '跟随系统',
  // 用户相关
  'user.logout': '退出登录',
  'user.defaultName': '用户',
  'user.changeEmail': '修改邮箱',
  'user.changeAvatar': '修改头像',
  'user.changePhone': '修改手机号',
  'user.changeRemark': '修改备注',
  'user.profile': '个人中心',
  'logout.success': '已退出登录',
  // 个人中心页
  'profile.title': '个人中心',
  'profile.basicInfo': '基本信息',
  'profile.accountInfo': '账户信息',
  'profile.identity': '身份信息',
  'profile.contact': '联系方式',
  'profile.name': '姓名',
  'profile.nickname': '昵称',
  'profile.phone': '手机号',
  'profile.uid': '用户ID',
  'profile.status': '状态',
  'profile.createdAt': '创建时间',
  'profile.updatedAt': '更新时间',
  'profile.joinedAt': '加入时间',
  'profile.lastUpdated': '上次变更',
  'profile.remark': '备注',
  'profile.changeAvatarTip': '点击修改头像',
  // Self 弹窗
  'self.email': '邮箱',
  'self.avatar': '头像',
  'self.emailPlaceholder': '请输入新邮箱',
  'self.avatarPlaceholder': '请输入头像地址',
  'self.phonePlaceholder': '请输入手机号',
  'self.remarkPlaceholder': '请输入备注',
  // 命名空间相关
  'namespace.select': '选择命名空间',
  // 语言相关
  'language.zh': '中文',
  'language.en': 'English',
  // 菜单相关
  'menu.goddess': '系统管理',
  'menu.profile': '个人中心',
  'menu.rabbit': '消息管理',
  'menu.rabbitTemplates': '模板列表',
  'menu.rabbitEmails': '邮件列表',
  'menu.rabbitWebhooks': '网络钩子',
  'menu.rabbitMessages': '消息日志',
  'menu.rabbitSender': '发送消息',
  'menu.rabbitRecipientGroups': '收件人组',
  'menu.rabbitAlertSubscriptions': '消息订阅',
  'menu.marksman': '策略管理',
  'menu.datasources': '数据源',
  'menu.strategyGroups': '策略组',
  'menu.strategies': '策略列表',
  'menu.levels': '等级列表',
  'menu.realtimeAlerts': '实时告警',
  'menu.historyAlerts': '历史告警',
  'menu.jadeTree': '节点探针',
  'menu.jadeTreeDashboard': '服务控制台',
  'menu.jadeTreeCommands': 'SSH命令',
  'menu.jadeTreeAudits': '审核记录',
  'menu.jadeTreeProbes': '探测任务',
  'menu.jadeTreeMachines': '机器信息',
  'menu.namespaces': '命名空间',
  'menu.users': '用户管理',
  'menu.members': '成员管理',
  'menu.settings': '设置',
  // 系统名称
  'layout.appName': 'Moon监控',
  // Footer
  'footer.copyright': 'Copyright© {year} MOON监控系统',
  'footer.icp': '贵公网安备52011502009111号 | 黔ICP备19012566号-8',
  // 通用操作
  'common.ok': '确定',
  'common.cancel': '取消',
  'common.close': '关闭',
  'common.submit': '提交',
  'common.search': '搜索',
  'common.reset': '重置',
  'common.add': '新增',
  'common.edit': '编辑',
  'common.delete': '删除',
  'common.detail': '详情',
  'common.view': '查看',
  'common.more': '更多',
  'common.copy': '复制',
  'common.copy.success': '复制成功',
  'common.copy.failed': '复制失败，请稍后重试',
  'common.export': '导出',
  'common.save': '保存',
  'common.back': '返回',
  'common.confirm': '确认',
  'common.loading': '加载中...',
  'common.noData': '暂无数据',
  'common.failed': '失败',
  'common.kv.add': '添加标签',
  'common.kv.empty': '未配置标签',
  'common.kv.keyPlaceholder': '键，例如 severity',
  'common.kv.valuePlaceholder': '值，例如 critical',
  'common.kv.keyRequired': '标签值存在时必须填写标签键',
  'common.kv.keyDuplicate': '标签键不能重复',
  'common.status': '状态',
  'common.status.ENABLED': '启用',
  'common.status.DISABLED': '禁用',
  'common.status.UNKNOWN': '未知',
  // 表格通用
  'table.action': '操作',
  'table.search.type': '类型',
  'table.search.keyword': '模糊查询',
  'table.search.placeholder': '请输入',
  'table.search.all': '全部',
  'table.total': '共 {total} 条',
  // 消息类型（MessageType 枚举，模板管理 / 消息日志共用）
  'messageType.UNKNOWN': '未知',
  'messageType.EMAIL': '邮件',
  'messageType.SMS_ALICLOUD': '短信(阿里云)',
  'messageType.WEBHOOK_OTHER': '网络钩子(其他)',
  'messageType.WEBHOOK_DINGTALK': '网络钩子(钉钉)',
  'messageType.WEBHOOK_WECHAT': '网络钩子(企业微信)',
  'messageType.WEBHOOK_FEISHU': '网络钩子(飞书)',
  // 通用消息
  'message.success': '操作成功',
  'message.error': '操作失败',
  'message.delete.success': '删除成功',
  'message.create.success': '创建成功',
  'message.update.success': '更新成功',
  // 登录/注册
  'login.welcomeBack': '欢迎回来',
  'login.welcomeSub': '登录以继续使用',
  'login.createAccount': '创建账号',
  'login.registerSub': '填写信息完成注册',
  'login.login': '登录',
  'login.register': '注册',
  'login.emailPlaceholder': '请输入邮箱',
  'login.emailRequired': '请输入邮箱',
  'login.emailInvalid': '请输入有效邮箱',
  'login.passwordPlaceholder': '请输入密码',
  'login.passwordRequired': '请输入密码',
  'login.confirmPasswordPlaceholder': '请再次输入密码',
  'login.confirmPasswordRequired': '请确认密码',
  'login.passwordMismatch': '两次密码不一致',
  'login.graphicCaptchaLabel': '图形验证码',
  'login.graphicCaptchaPlaceholder': '请输入图形验证码',
  'login.graphicCaptchaRequired': '请输入图形验证码',
  'login.enterCaptchaFirst': '请先输入图形验证码',
  'login.captchaError': '图形验证码错误',
  'login.captchaModalTitle': '安全验证',
  'login.captchaModalConfirm': '确认',
  'login.graphicCaptchaHint': '输入正确后将向邮箱发送验证码',
  'login.emailCodePlaceholder': '请输入邮箱验证码',
  'login.emailCodeRequired': '请输入邮箱验证码',
  'login.sendCode': '获取验证码',
  'login.resendIn': '{count}秒后重发',
  'login.codeSent': '验证码已发送',
  'login.loginSuccess': '登录成功',
  'login.switchToCode': '邮箱验证码登录',
  'login.switchToPassword': '密码登录',
  'login.submitHint': '登录接口需对接后端',
  'login.registerSubmitHint': '注册接口需对接后端',
  'login.otherWays': '其他登录方式',
  'login.other.wechat': '微信',
  'login.other.qq': 'QQ',
  'login.other.github': 'GitHub',
  'login.other.google': 'Google',
  'login.other.gitee': 'Gitee',
  'login.other.feishu': '飞书',
  'login.intro.title1': 'MOON 监控平台',
  'login.intro.desc1': '一站式监控与告警，让运维更简单。',
  'login.intro.title2': '多维度监控',
  'login.intro.desc2': '支持基础设施、应用、业务等多维度监控与可视化。',
  'login.intro.title3': '智能告警',
  'login.intro.desc3': '灵活规则与多渠道通知，第一时间发现并处理异常。',
} as const

export const enUS = {
  // Theme related
  'theme.light': 'Light Theme',
  'theme.dark': 'Dark Theme',
  'theme.system': 'Follow System',
  // User related
  'user.logout': 'Logout',
  'user.defaultName': 'User',
  'user.changeEmail': 'Change Email',
  'user.changeAvatar': 'Change Avatar',
  'user.changePhone': 'Change Phone',
  'user.changeRemark': 'Change Remark',
  'user.profile': 'Personal Center',
  'logout.success': 'Logged out successfully',
  // Profile page
  'profile.title': 'Personal Center',
  'profile.basicInfo': 'Basic Info',
  'profile.accountInfo': 'Account Info',
  'profile.identity': 'Identity',
  'profile.contact': 'Contact',
  'profile.name': 'Name',
  'profile.nickname': 'Nickname',
  'profile.phone': 'Phone',
  'profile.uid': 'User ID',
  'profile.status': 'Status',
  'profile.createdAt': 'Created At',
  'profile.updatedAt': 'Updated At',
  'profile.joinedAt': 'Joined',
  'profile.lastUpdated': 'Last updated',
  'profile.remark': 'Remark',
  'profile.changeAvatarTip': 'Click to change avatar',
  // Self modals
  'self.email': 'Email',
  'self.avatar': 'Avatar',
  'self.emailPlaceholder': 'Enter new email',
  'self.avatarPlaceholder': 'Enter avatar URL',
  'self.phonePlaceholder': 'Enter phone number',
  'self.remarkPlaceholder': 'Enter remark',
  // Namespace related
  'namespace.select': 'Select Namespace',
  // Language related
  'language.zh': '中文',
  'language.en': 'English',
  // Menu related
  'menu.goddess': 'System',
  'menu.profile': 'Personal Center',
  'menu.rabbit': 'Messages',
  'menu.rabbitTemplates': 'Templates',
  'menu.rabbitEmails': 'Emails',
  'menu.rabbitWebhooks': 'Webhooks',
  'menu.rabbitMessages': 'Log',
  'menu.rabbitSender': 'Send',
  'menu.rabbitRecipientGroups': 'Recipient Groups',
  'menu.rabbitAlertSubscriptions': 'Message Subscriptions',
  'menu.marksman': 'Policies',
  'menu.datasources': 'Datasources',
  'menu.strategyGroups': 'Strategy Groups',
  'menu.strategies': 'Strategies',
  'menu.levels': 'Levels',
  'menu.realtimeAlerts': 'Real-time Alerts',
  'menu.historyAlerts': 'Alert History',
  'menu.jadeTree': 'Jade Tree',
  'menu.jadeTreeDashboard': 'Console',
  'menu.jadeTreeCommands': 'SSH Commands',
  'menu.jadeTreeAudits': 'Audit Records',
  'menu.jadeTreeProbes': 'Probe Tasks',
  'menu.jadeTreeMachines': 'Machine Info',
  'menu.namespaces': 'Namespaces',
  'menu.users': 'Users',
  'menu.members': 'Members',
  'menu.settings': 'Settings',
  // 系统名称
  'layout.appName': 'Moon Monitor',
  // Footer
  'footer.copyright': 'Copyright© {year} MOON Monitoring System',
  'footer.icp':
    'Gui Gong Network Security 52011502009111 | Qian ICP No. 19012566-8',
  // Common actions
  'common.ok': 'OK',
  'common.cancel': 'Cancel',
  'common.close': 'Close',
  'common.submit': 'Submit',
  'common.search': 'Search',
  'common.reset': 'Reset',
  'common.add': 'Add',
  'common.edit': 'Edit',
  'common.delete': 'Delete',
  'common.detail': 'Detail',
  'common.view': 'View',
  'common.more': 'More',
  'common.copy': 'Copy',
  'common.copy.success': 'Copied',
  'common.copy.failed': 'Copy failed, please try again later',
  'common.export': 'Export',
  'common.save': 'Save',
  'common.back': 'Back',
  'common.confirm': 'Confirm',
  'common.loading': 'Loading...',
  'common.noData': 'No Data',
  'common.failed': 'Failed',
  'common.kv.add': 'Add label',
  'common.kv.empty': 'No labels configured',
  'common.kv.keyPlaceholder': 'Key, e.g. severity',
  'common.kv.valuePlaceholder': 'Value, e.g. critical',
  'common.kv.keyRequired': 'A key is required when a value is present',
  'common.kv.keyDuplicate': 'Label keys must be unique',
  'common.status': 'Status',
  'common.status.ENABLED': 'Enabled',
  'common.status.DISABLED': 'Disabled',
  'common.status.UNKNOWN': 'Unknown',
  // Table common
  'table.action': 'Action',
  'table.search.keyword': 'Fuzzy search',
  'table.search.placeholder': 'Please enter',
  'table.search.type': 'Type',
  'table.search.all': 'All',
  'table.total': 'Total {total} items',
  // Message type (MessageType enum, shared by template & message log)
  'messageType.UNKNOWN': 'Unknown',
  'messageType.EMAIL': 'Email',
  'messageType.SMS_ALICLOUD': 'SMS(Alicloud)',
  'messageType.WEBHOOK_OTHER': 'Webhooks(Other)',
  'messageType.WEBHOOK_DINGTALK': 'Webhooks(DingTalk)',
  'messageType.WEBHOOK_WECHAT': 'Webhooks(WeChat)',
  'messageType.WEBHOOK_FEISHU': 'Webhooks(Feishu)',
  // Common messages
  'message.success': 'Operation successful',
  'message.error': 'Operation failed',
  'message.delete.success': 'Deleted successfully',
  'message.create.success': 'Created successfully',
  'message.update.success': 'Updated successfully',
  // Login / Register
  'login.welcomeBack': 'Welcome back',
  'login.welcomeSub': 'Sign in to continue',
  'login.createAccount': 'Create account',
  'login.registerSub': 'Fill in to complete registration',
  'login.login': 'Sign in',
  'login.register': 'Register',
  'login.emailPlaceholder': 'Email',
  'login.emailRequired': 'Please enter email',
  'login.emailInvalid': 'Please enter a valid email',
  'login.passwordPlaceholder': 'Password',
  'login.passwordRequired': 'Please enter password',
  'login.confirmPasswordPlaceholder': 'Confirm password',
  'login.confirmPasswordRequired': 'Please confirm password',
  'login.passwordMismatch': 'Passwords do not match',
  'login.graphicCaptchaLabel': 'Captcha',
  'login.graphicCaptchaPlaceholder': 'Enter captcha',
  'login.graphicCaptchaRequired': 'Please enter captcha',
  'login.enterCaptchaFirst': 'Enter captcha first',
  'login.captchaError': 'Wrong captcha',
  'login.captchaModalTitle': 'Security verification',
  'login.captchaModalConfirm': 'Confirm',
  'login.graphicCaptchaHint':
    'After entering correctly, a code will be sent to your email.',
  'login.emailCodePlaceholder': 'Email code',
  'login.emailCodeRequired': 'Please enter email code',
  'login.sendCode': 'Get code',
  'login.resendIn': 'Resend in {count}s',
  'login.codeSent': 'Code sent',
  'login.loginSuccess': 'Login successful',
  'login.switchToCode': 'Sign in with email code',
  'login.switchToPassword': 'Sign in with password',
  'login.submitHint': 'Login API needs backend',
  'login.registerSubmitHint': 'Register API needs backend',
  'login.otherWays': 'Other sign-in options',
  'login.other.wechat': 'WeChat',
  'login.other.qq': 'QQ',
  'login.other.github': 'GitHub',
  'login.other.google': 'Google',
  'login.other.gitee': 'Gitee',
  'login.other.feishu': 'Feishu',
  'login.intro.title1': 'MOON Monitoring',
  'login.intro.desc1': 'One-stop monitoring and alerting.',
  'login.intro.title2': 'Multi-dimensional',
  'login.intro.desc2': 'Infrastructure, application and business monitoring.',
  'login.intro.title3': 'Smart alerts',
  'login.intro.desc3': 'Flexible rules and multi-channel notifications.',
} as const
